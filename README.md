...
| 工具 | 功能 |
|------|------|
| `read_file` | 读取文件（支持分段、行号标注，图片自动识别） |
| `write_file` | 创建/覆盖文件（写入后自动语法检查） |
| `apply_patch` | 精准修改文件中的一段文本（SEARCH/REPLACE） |
| `delete_file` | 删除文件 |
| `search` | 全文搜索（关键词/正则，上下文预览，文件级统计） |
| `search_file` | 按文件名模式搜索 |
| `tree` | 递归目录树 |
| `bash` | 执行 Shell 命令（Windows 自适应 PowerShell） |
| `web_search` | 搜索互联网 |
| `web_fetch` | 抓取网页内容（自动分页） |
| `rename_symbol` | 符号重命名（单词边界匹配，不误伤子串，自动语法检查） |
...
| 工具 | 描述 | 反馈质量 |
|------|------|---------|
| `read_file` | 带行号、分段读取，图片自动识别为视觉数据 | ⭐ 优秀：总行数、剩余行提示 |
| `apply_patch` | SEARCH/REPLACE 精准编辑，唯一性检查，写入后自动 lint | ⭐ 优秀：变更行号、语法检查结果 |
| `write_file` | 全量写入，支持新建和覆盖，自动语法检查 | ⭐ 优秀：行数统计 + lint |
| `delete_file` | 删除指定文件 | ✅ 可靠 |
| `rename_symbol` | 单词边界正则匹配，跨文件全部替换，自动 lint | ⭐ 优秀：行级明细 + 语法检查 |

### 搜索工具
| 工具 | 描述 | 反馈质量 |
|------|------|---------|
| `search` | grep 模式搜索，支持上下文预览 | ⭐ 优秀：文件级统计、500 条上限 |
...
| 工具 | 描述 | 反馈质量 |
|------|------|---------|
| `bash` | Shell 命令执行，3 分钟超时 | ⭐ 优秀：exit code、输出、中断支持 |

### 网络工具
| 工具 | 描述 | 反馈质量 |
|------|------|---------|
| `web_search` | 互联网搜索（DuckDuckGo） | ✅ 可靠：标题/链接/摘要 |
| `web_fetch` | 抓取网页内容 | ⭐ 优秀：自动分页、字符编码适配 |
...
│  ┌─────────────────┐         ┌────────────────────┐  │
│  │ 工具路由处理      │         │ 第三方 MCP 服务器   │  │
│  │ read/write/search│         │ (PDF/Excel/浏览器等)│  │
│  │ bash/web/tree... │         └────────────────────┘  │
│  └─────────────────┘                                  │
└─────────────────────────────────────────────────────┘
        │
...
4. **工具循环** → AI 返回 `tool_calls`，前端并行执行所有工具调用
5. **结果回传** → 每个工具的执行结果返回给 AI，AI 决定下一步（最多 35 轮）
...
- **工作空间沙箱**：所有文件操作和 bash 命令只能在当前工作空间目录内执行
- **路径安全检查**：每步操作都经过 `safePath()` 校验，防止路径穿越
- **工具开关**：每个内置工具可独立关闭，可在不重启应用的情况下限制 AI 能力
- **命令超时**：所有 bash 命令有 3 分钟超时，自动终止
- **中断机制**：随时中断 AI 回复，同时终止后台子进程
- **单实例锁**：防止应用重复启动
...
<p align="center">
  用 ❤️ 和 AI 做的桌面小伙伴(如果这个项目对你有启发，欢迎。如果有合作机会，联系微信：zhengtu920)
</p>
